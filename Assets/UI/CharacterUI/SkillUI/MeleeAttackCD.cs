...
public class MelleAttackCD : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        float CDTime = Mathf.Min(attack.meleeAttackCooldown, attack.meleeCooldownTimer);
        CDTime = attack.meleeAttackCooldown - CDTime;


        display.text = (CDTime > 0) ? string.Format("{0:F1}", CDTime) : "Ready!";
        float sizeRatio = 1 - (CDTime / attack.meleeAttackCooldown);
